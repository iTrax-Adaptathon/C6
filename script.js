...
function dashboard(result) {
  ...
  app.innerHTML = `<section class="page"><header class="topbar"><div class="brand"><i>±</i> AI Debate Judge</div><button class="ghost" id="reset">← New debate</button></header><section class="card banner"><div class="banner-left"><div class="icon-box">⚖</div><div><span class="badge">JUDGMENT DELIVERED</span><h1>${winner} wins</h1><p class="sub">A clearer case on balance of argument.</p></div></div><div class="score"><small>OVERALL SCORE</small><strong>${a.overall.toFixed(1)} <span>vs</span> ${b.overall.toFixed(1)}</strong></div></section><section class="overview"><article class="card pad"><p class="eyebrow">Argument profile</p><h2 class="section-title">How the cases compare</h2><svg class="chart" viewBox="0 0 300 270"><g transform="translate(0,5)" fill="none" stroke="#dbe3dc"><polygon points="150,42 226,174 74,174"/><polygon points="150,64 207,163 93,163"/><polygon points="150,86 188,152 112,152"/></g><g transform="translate(0,5)" font-family="DM Sans" font-size="11" font-weight="700" fill="#69736b"><text x="132" y="18">Logic</text><text x="237" y="190">Evidence</text><text x="21" y="190">Rebuttal</text></g><polygon transform="translate(0,5)" points="${polygon([profile.logic.a, profile.evidence.a, profile.rebuttal.a])}" fill="#3a5c4738" stroke="#3a5c47" stroke-width="2"/><polygon transform="translate(0,5)" points="${polygon([profile.logic.b, profile.evidence.b, profile.rebuttal.b])}" fill="#9aa69d25" stroke="#9aa69d" stroke-width="2"/></svg><div class="legend"><span><i class="dot"></i>Side A</span><span><i class="dot gray"></i>Side B</span></div></article><article class="card pad"><p class="eyebrow">The motion</p><h2 class="section-title">${escapeHtml(result.motion)}</h2><p class="ruling-quote">“${escapeHtml(result.pull_quote)}”</p><p class="sub">Explore each claim below to see how the ruling was reached.</p></article></section><section class="claims-header"><p class="eyebrow">Claim analysis</p><h2 class="section-title">Evidence, logic & response</h2><div class="claims">${claimColumn('Side A', a)}${claimColumn('Side B', b)}</div></section><section class="final"><p class="eyebrow">Final verdict</p><p>${escapeHtml(result.final_verdict)}</p></section><button class="primary again" id="again">Judge another debate →</button></section>`;
